 ...
 */
class SingleChoiceDialog : BaseDialog() {
    /**
     * ダイアログに適用するアダプター
     *
     * 結果送信時、回転時に選択肢を復元する際に使用する
     */
    private lateinit var dialogAdapter: SingleChoiceItemAdapter

    companion object {
        fun newInstance() = SingleChoiceDialog()
         ...
         * 選択していたインデックスの数値
         */
        const val SAVED_SELECT_INDEX = "savedSelectIndex"
    ...
    class Builder: BaseDialog.Builder() {
         ...
         */
        override fun build(): SingleChoiceDialog {
            // タイトルが存在しない場合
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        if (savedInstanceState?.getInt(SAVED_SELECT_INDEX) != null) {
            selectorItemArrayList!!.forEachIndexed { index, selectorItem ->
                selectorItem.isChecked = savedInstanceState.getInt(SAVED_SELECT_INDEX) == index
            }
        }
        this.dialogAdapter = SingleChoiceItemAdapter(
    ...
    override fun onSaveInstanceState(outState: Bundle) {
        ...
        outState.putInt(SAVED_SELECT_INDEX, this.getAdapterSelectedIndex())
    }

    /**
     * Activityから呼び出したときのコールバック処理を指定する。
     *
     * bundle.getInt(SELECTED_INDEX)で選択したインデックスを取得することができる。
     *
    ...
    override fun sendDialogResultToActivity(buttonVal: Int) {
        ...
        selectedBundle.putInt(SELECTED_INDEX, this.getAdapterSelectedIndex())
    ...
    override fun sendDialogResultToFragment(buttonVal: Int) {
        ...
        resultArgs.putInt(SELECTED_INDEX, this.getAdapterSelectedIndex())
        setFragmentResult(
            arguments?.getString(DIALOG_REQUEST_KEY)!!,
            resultArgs
        )
     ...
     * 現在選択しているインデックスを取得する
     *
     * @return SingleSelectorItemAdapter.radioButtonCheckPositionを参照
     */
    private fun getAdapterSelectedIndex(): Int = this.dialogAdapter.radioButtonCheckPosition
}